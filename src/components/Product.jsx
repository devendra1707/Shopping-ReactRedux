...
const Product = ({ post }) => {
  ...
  const addToCart = () => {
    dispatch(add(post));
    toast.error("Item added Successful");
  };

  return (
    <div className="flex flex-col items-center justify-between hover:scale-110 trasition duration-300 ease-in p-4 ml-5 gap-10 rounded-xl ">
      <div>
        ...
        {cart.some((p) => p.id == post.id) ? (
          <button
            className="text-gray-700 border-2 border-gray-700 rounded-full
             font-semibold text-[12px] p-1 px-3 
             uppercase hover:bg-gray-700
